...
int main()
{
   ...
   const char *valueOne = dictGet(&dictionary, aKey);

   printf("Key: '%s' %s found\n", aKey, valueOne ? "was" : "was not");
   
   const char *badKey   = "some-non-existent-key";
   const char *valueTwo = dictGet(&dictionary, badKey);

   printf("Key: '%s' %s found\n", badKey, valueTwo ? "was" : "was not");

...
typedef void (*FunctionPointer)(Pair *pairPtr);

static void dictForEach(Dictionary *dictPtr, FunctionPointer fp)
{
   for(int i = 0; i < dictPtr->mSize; i ++)
   {
      Node *n = dictPtr->mNodes[i];
...
static void capitalizeValue(Pair *pairPtr)
{
   for(int i = 0; pairPtr->mValue[i]; i ++)
      pairPtr->mValue[i] = (char)toupper(pairPtr->mValue[i]);
}

static void showKeyAndValue(Pair *pairPtr)
{
   printf("Key: '%s', Value: '%s'\n", pairPtr->mKey, pairPtr->mValue);
}
...
int main()
{
   ...
   dictForEach(&dictionary, showKeyAndValue);

   printf("\n");

   dictForEach(&dictionary, capitalizeValue);

...
static void loadDictionary(Dictionary *ptr, char *aKey)
{
   const int FACTOR = 10;
   const int LENGTH = 10;
...
int main()
{
   ...
   char aKey[BUFFER_SIZE] = { '\0' };

   loadDictionary(&dictionary, aKey);

   printf("\n");

   const char *value = dictGet(&dictionary, aKey);

   if(!value)
   {
      printf("Cound not find key: '%s'\n", aKey);
   }
   else
   {
      ...
      if(!dictRemove(&dictionary, aKey))
      {
         printf("Could not remove key: '%s'\n", aKey);
      }
      else
      {
         printf("Removed key: '%s'\n", aKey);

         if(dictGet(&dictionary, aKey))
            printf("Error: key '%s' still present\n", aKey);

         else
            printf("Success: key '%s' was removed\n",  aKey);
      }
   }

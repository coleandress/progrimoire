...
{
   *aCountPtr = *eCountPtr = *iCountPtr = *oCountPtr = *uCountPtr;
   ...
   for(int i = 0; i < dictPtr->mSize; i ++)
   {
      ...
      while(n)
      {
         for(int j = 0; n->mPair.mKey[j]; j ++)
         {
            switch(n->mPair.mKey[j])
            {
               case 'a': ++(*aCountPtr); break;
               case 'e': ++(*eCountPtr); break;
               case 'i': ++(*iCountPtr); break;
               case 'o': ++(*oCountPtr); break;
               case 'u': ++(*uCountPtr); break;
            }
         }
...
int main()
{
   ...
   dictCountVowels(&dictionary, &aCount, &eCount, &iCount, &oCount, &uCount);

   printf(
      "In our dictionary's keys, we found: %d a%s, %d e%s, %d i%s, %d o%s, %d u%s\n",
      aCount, aCount == 1 ? "" : "'s",
      eCount, eCount == 1 ? "" : "'s",
      iCount, iCount == 1 ? "" : "'s",
      oCount, oCount == 1 ? "" : "'s",
